  ...
  public constructor(proxyHandler: PlayerProxy) {
    ...
    proxyHandler.on('fromServer', async ({ name, data }) => {
      ...
      if (name === 'named_entity_spawn') {
        this.emit('player_spawn', data.playerUUID, data.entityId, parseLocation(data));
      }

      if (name === 'player_info' && data.action === 0) {
        for (const player of data.data.filter(i => i.UUID && i.name)) this.emit('player_join', player.UUID, player.name);
      }
