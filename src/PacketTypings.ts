...
export type PacketsPlayToClient = {
  ...
  entity_move_look: PacketsPlayToClient['rel_entity_move'] & PacketsPlayToClient['entity_look'];
  ...
  open_window: ({ inventoryType: string } | { inventoryType: 'EntityHorse'; entityId: number }) & {
    windowId: number;
    windowTitle: string;
    slotCount: number;
  };
  ...
  scoreboard_objective: {
    name: string;
  } & ({ action: number } | { action: 0 | 2; displayText: string; type: string });
  scoreboard_score: {
    itemName: string;
    scoreName: string;
